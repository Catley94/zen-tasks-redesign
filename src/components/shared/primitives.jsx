...
import {
  TOKENS, btnReset, PRIORITY_LABEL, PRIORITY_COLOR,
  lastActiveDays, MAX_SIDEBAR_PROJECTS, PROJECT_STALE_DAYS,
  ZEN_NUDGES, NOTIFICATIONS, CATEGORIES, NOTE_COLLECTIONS,
  GOALS, PROJECTS,
} from '../../data/seeds.js';
import {
  Leaf, Sparkles, User, Check, ChevR, ChevD, Moon, Plus, Close,
  Home, Target, Folder, Settings, Flame, Dot, ListTodo, Arr, MoreV,
  Pin, Bell, Search, Grid, Tag, Heart, Bag, Wallet, Compass, Bookmark,
  CalToday, CalMonth, Trash, NotebookIcon,
} from './icons.jsx';
import { useZenAI } from '../../state/useAppState.jsx';

// ——— small bits ———
export function PriorityDot({ p, size = 8 }) {
...
export function QuietBadge({ children = 'gently quiet' }) {
...
export const StaleBadge = QuietBadge;

export function FocusPill({ on, onClick, size = 'sm' }) {
  const s = size === 'sm';
  return (
...
export function GoalChip({ goal, primary = false, compact = false, onClick }) {
  if (!goal) return null;
  return (
...
export function SectionLabel({ children, color = TOKENS.sub }) {
...
export function TagPill({ tag }) {
  const T = TOKENS;
  return (
    <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 999, background: T.bgSoft, border: `1px solid ${T.line}`, color: T.sub }}>{tag}</span>
  );
}

export function CatIcon({ icon, size = 14 }) {
  const icons = { bag: Bag, home: Home, heart: Heart, wallet: Wallet, tag: Tag, compass: Compass, bookmark: Bookmark, leaf: Leaf };
  const I = icons[icon] || Tag;
  return <I size={size}/>;
}
...
export function TaskRowMenu({ task, app, hover = false }) {
...
export function TaskRow({ task, app, showProject = false, showGoal = false, compact = false, highlighted = false, mobile = false }) {
  ...
  const hasMeta = (showProject && project) || taskGoals.length > 0 || tags.length > 0;
...
export function AddToFocusModal({ open, onClose, app }) {
  ...
  const filterLabel = () => {
    ...
    if (filter.startsWith('p:')) return (app.projects.find(p => p.id === filter.slice(2))||{}).name;
    return 'All tasks';
  };
  const filterColor = () => {
    if (filter.startsWith('g:')) return (goals.find(g => g.id === filter.slice(2))||{}).color;
    if (filter.startsWith('p:')) return (app.projects.find(p => p.id === filter.slice(2))||{}).color;
    return null;
  };
                  ...
                  <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, right: 0, zIndex: 2,
                    ...
                    {goals.map(g => (
                      <FilterOption key={g.id} active={filter==='g:'+g.id} onClick={()=>pick('g:'+g.id)}
                        ...
                        label={g.name}
                        count={eligibleAll.filter(t=>{const pr=app.projects.find(p=>p.id===t.projectId); return pr && (pr.goalIds||[]).includes(g.id);}).length}/>
                    ))}
...
// ——— Ask Zen overlay (summoned) — driven by app.zenChat, can open pre-seeded ———
export function AskZenOverlay({ app, open: openProp, onClose: onCloseProp }) {
  ...
  // Fresh contextual open: reset the thread and auto-send the seeded prompt (if any).
  useEffect(() => {
    if (!open) return;
    if (chat.nonce === handledNonce.current) return;
    handledNonce.current = chat.nonce;
    if (chat.seed) { reset(); const s = chat.seed; setTimeout(()=>send(s), 0); }
    else if (chat.label) { reset(); }
  }, [open, chat.nonce]);
  useEffect(() => {
  ...
  const suggestions = (chat.suggestions && chat.suggestions.length) ? chat.suggestions : ["What should I start with today?", "Any projects I'm avoiding?", "Remind me why this matters."];
  return (
    <div onClick={close} style={{ position: 'absolute', inset: 0, background: 'rgba(30,40,30,0.22)',
      backdropFilter: 'blur(6px)', display: 'grid', placeItems: 'start center', paddingTop: 90, zIndex: 140 }}>
      <div onClick={e=>e.stopPropagation()} style={{ width: 'min(520px, 92%)', maxHeight: 'calc(100% - 120px)',
        ...
        {chat.label && (
          <div style={{ padding: '8px 18px', borderBottom: `1px solid ${TOKENS.lineSoft}`, display: 'flex', alignItems: 'center', gap: 8, background: TOKENS.tealTint+'66' }}>
            <span style={{ fontSize: 9.5, textTransform: 'uppercase', letterSpacing: 1, color: TOKENS.tealDeep, fontWeight: 600, flexShrink: 0 }}>Talking about</span>
            <span style={{ fontSize: 12.5, color: TOKENS.ink, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{chat.label}</span>
          </div>
        )}
        ...
        <div style={{ padding: '10px 14px 14px', borderTop: `1px solid ${TOKENS.line}`, display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {suggestions.map(s => (
            <button key={s} onClick={() => send(s)} disabled={busy}
              style={{ ...btnReset, padding: '6px 10px', borderRadius: 999, background: TOKENS.bg, border: `1px solid ${TOKENS.line}`, color: TOKENS.sub, fontSize: 11.5, cursor: 'pointer' }}>{s}</button>
          ))}
        </div>
      </div>
    </div>
...
// ——— Ambient "line of Zen": a calm sentence + context-loaded chips into the chat ———
export function ZenLine({ app, children, chips = [] }) {
  return (
    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', background: TOKENS.tealTint+'66',
      border: `1px solid ${TOKENS.tealSoft}`, borderRadius: TOKENS.radius, padding: '12px 14px' }}>
      <span style={{ width: 26, height: 26, borderRadius: 999, background: TOKENS.teal, color: '#fff', display: 'grid', placeItems: 'center', flexShrink: 0 }}><Sparkles size={12}/></span>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, lineHeight: 1.55, color: TOKENS.ink }}>{children}</div>
        {chips.length > 0 && (
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 9 }}>
            {chips.map((c, i) => (
              <button key={i} onClick={()=>app.openZenChat && app.openZenChat({ seed: c.seed, context: c.context, label: c.label, suggestions: c.suggestions })}
                style={{ ...btnReset, display: 'inline-flex', alignItems: 'center', gap: 6, padding: '5px 11px', borderRadius: 999,
                  background: TOKENS.surface, border: `1px solid ${TOKENS.tealSoft}`, color: TOKENS.tealDeep, fontSize: 11.5, fontWeight: 500, cursor: 'pointer' }}
                onMouseEnter={e=>e.currentTarget.style.background = TOKENS.tealTint} onMouseLeave={e=>e.currentTarget.style.background = TOKENS.surface}>
                <Sparkles size={11}/>{c.chip}<Arr size={11} style={{ opacity: 0.6 }}/>
              </button>
            ))}
          </div>
        )}
...
export function ZenCorner({ app, onExpand }) {
...
export function CommandPalette({ open, onClose, app, onNav }) {
...
export function ProfileManager({ app, onClose }) {
...
export function ProfileSwitcher({ app, compact }) {
  const [open, setOpen] = useState(false);
...
export function buildProjectNav(app) {
...
// ——— Sidebar (desktop) ———
export function Sidebar({ active, onNav, app, onSearch }) {
  ...
  const noteCollChildren = (app?.collections || NOTE_COLLECTIONS).map(c => ({ id: 'notes:'+c.id, name: c.name, color: c.color }));
  const items = [
    ...
    { id: 'notes', name: 'Notes', icon: NotebookIcon, children: noteCollChildren },
...
export function NotificationToast({ app }) {
...
export function NotificationsBell({ app, align = 'right', narrow = false, onNav }) {
  const [open, setOpen] = useState(false);
          ...
          <div style={{ position: 'absolute', top: 'calc(100% + 8px)', zIndex: 71,
            ...
            <div style={{ maxHeight: 400, overflowY: 'auto' }}>
              {items.map(n => {
                ...
                // Resolve any task / project / goal this notification points at into nav buttons.
                const refs = [];
                if (n.ctaTaskId && app && app.tasks) { const t = app.tasks.find(x => x.id === n.ctaTaskId); if (t) refs.push({ dot: TOKENS.green, label: t.title, kind: 'Task', run: () => { app.openTask(t.id); setOpen(false); } }); }
                if (n.ctaProjectId && app && app.projectById) { const p = app.projectById(n.ctaProjectId); if (p) refs.push({ dot: p.color, label: p.name, kind: 'Project', run: () => { onNav && onNav('project:' + p.id); setOpen(false); } }); }
                if (n.ctaGoalId && app && app.goalById) { const g = app.goalById(n.ctaGoalId); if (g) refs.push({ icon: Target, color: g.color, label: g.name, kind: 'Goal', run: () => { onNav && onNav('goal:' + g.id); setOpen(false); } }); }
                if (n.ctaNav) refs.push({ icon: NOTIF_ICON[n.kind] ? meta.Icon : Settings, color: TOKENS.sub, label: n.ctaNavLabel || 'Open', kind: 'Go', run: () => { onNav && onNav(n.ctaNav); setOpen(false); } });
                return (
                  <div key={n.id} style={{ borderTop: `1px solid ${TOKENS.lineSoft}`, background: !n.read ? TOKENS.greenTint+'66' : 'transparent' }}>
                    ...
                    {isOpen && (
                      <div style={{ padding: '0 16px 13px 55px' }}>
                        <div style={{ fontSize: 12.5, color: TOKENS.inkSoft, lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{n.body}</div>
                        {refs.length > 0 && (
                          <div style={{ marginTop: 11, display: 'flex', flexDirection: 'column', gap: 6 }}>
                            <div style={{ fontSize: 9.5, color: TOKENS.sub, textTransform: 'uppercase', letterSpacing: 1.1, fontWeight: 500 }}>Jump to</div>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                              {refs.map((r, i) => (
                                <button key={i} onClick={(e)=>{ e.stopPropagation(); r.run(); }}
                                  style={{ ...btnReset, display: 'inline-flex', alignItems: 'center', gap: 7, padding: '6px 11px', borderRadius: 999, background: TOKENS.bg, border: `1px solid ${TOKENS.line}`, fontSize: 11.5, color: TOKENS.ink, cursor: 'pointer', maxWidth: '100%' }}
                                  onMouseEnter={e=>{ e.currentTarget.style.background = TOKENS.greenTint; e.currentTarget.style.borderColor = TOKENS.greenSoft; }}
                                  onMouseLeave={e=>{ e.currentTarget.style.background = TOKENS.bg; e.currentTarget.style.borderColor = TOKENS.line; }}>
                                  {r.dot ? <span style={{ width: 8, height: 8, borderRadius: 999, background: r.dot, flexShrink: 0 }}/> : r.icon ? <r.icon size={12} style={{ color: r.color, flexShrink: 0 }}/> : null}
                                  <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.label}</span>
                                  <Arr size={11} style={{ color: TOKENS.sub, flexShrink: 0 }}/>
                                </button>
                              ))}
                            </div>
                          </div>
...
export function TopBar({ crumbs, title, subtitle, right, app, onAsk, onNav }) {
  return (
    <header style={{ padding: '18px 30px 14px', borderBottom: `1px solid ${TOKENS.line}`,
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <NotificationsBell app={app} onNav={onNav}/>
        ...
        <button data-tour="zen" onClick={onAsk} aria-hidden={app.density === 'full'} tabIndex={app.density === 'full' ? -1 : 0} style={{ ...btnReset, width: 32, height: 32, borderRadius: 999,
          background: TOKENS.bg, border: `1px solid ${TOKENS.line}`, color: TOKENS.teal,
          display: 'grid', placeItems: 'center',
          visibility: app.density === 'full' ? 'hidden' : 'visible', pointerEvents: app.density === 'full' ? 'none' : 'auto' }} title="Ask Zen (⌘K)"><Sparkles size={14}/></button>
