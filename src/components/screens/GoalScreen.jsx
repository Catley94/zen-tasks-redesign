...
import { SectionLabel, GoalChip, ZenLine } from '../shared/primitives.jsx';
import { Plus, Target, Check, ChevD, ChevR, Arr, Edit, Close, Sparkles, Moon } from '../shared/icons.jsx';

function QuietBadge() {
  return <span style={{ fontSize: 10, padding: '2px 7px', borderRadius: 999, background: TOKENS.tealSoft, color: TOKENS.tealDeep }}>quiet</span>;
}
...
function LinkProjectsModal({ open, onClose, goal, app }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(30,40,30,0.32)', backdropFilter: 'blur(4px)', display: 'grid', placeItems: 'center', zIndex: 90 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: 'min(520px, 92%)', maxHeight: 'min(620px, 88%)', background: TOKENS.surface, borderRadius: TOKENS.radiusLg, border: `1px solid ${TOKENS.line}`, boxShadow: '0 40px 90px rgba(20,30,20,0.22)', display: 'flex', flexDirection: 'column', overflow: 'hidden', fontFamily: TOKENS.fontSans, color: TOKENS.ink }}>
        ...
        <div style={{ flex: 1, overflow: 'auto' }}>
          {(app.projects || []).map(p => {
            ...
            return (
              <label key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 18px', borderTop: `1px solid ${TOKENS.lineSoft}`, cursor: 'pointer', background: checked ? TOKENS.greenTint : 'transparent' }}>
                <button type="button" onClick={() => toggle(p.id)} style={{ ...btnReset, width: 18, height: 18, borderRadius: 4, border: `1.5px solid ${checked ? TOKENS.green : TOKENS.line}`, background: checked ? TOKENS.green : 'transparent', color: '#fff', display: 'grid', placeItems: 'center' }}>
...
export default function GoalScreen() {
  ...
  const isPrimary = goal && app.primaryGoalId === goal.id;

  const validPhase = (pid) => goal && goal.phases.some(p => p.id === pid) ? pid : null;
  const [phase, setPhase] = useState(() => goal?.phases[0]?.id);
  useEffect(() => {
    if (goal) setPhase(goal.phases[0]?.id);
  }, [goal?.id]);
  ...
  if (noGoals || !goal) {
    return (
      <Screen app={app} active="goal" onNav={onNav} frame={frame} title="Goals" crumbs={['Goals']}>
        <div style={{ padding: '60px 30px', display: 'grid', placeItems: 'center' }}>
          <div style={{ maxWidth: 380, textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 13, color: TOKENS.sub, lineHeight: 1.6 }}>Goals are the big things you're moving toward. Add projects and tasks to get started.</div>
          </div>
        </div>
      </Screen>
    );
  ...
  const headerRight = (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <button onClick={() => !isPrimary && app.setPrimaryGoalId(goal.id)} disabled={isPrimary}
        style={{ ...btnReset, padding: '6px 12px', borderRadius: 999, fontSize: 11.5, background: isPrimary ? TOKENS.greenSoft : 'transparent', color: isPrimary ? TOKENS.green : TOKENS.sub, border: `1px solid ${isPrimary ? TOKENS.greenSoft : TOKENS.line}`, display: 'inline-flex', alignItems: 'center', gap: 5, cursor: isPrimary ? 'default' : 'pointer' }}>
  ...
  return (
    <Screen app={app} active={'goal:' + goal.id} onNav={onNav} frame={frame}
      title={goal.name} subtitle={goal.overarching}
      crumbs={['Goals', isPrimary ? 'Primary' : 'Other']}
      ...
      <div style={{ padding: '22px 30px 40px', display: 'flex', flexDirection: 'column', gap: 18 }}>
        <div style={{ background: TOKENS.surfaceAlt, border: `1px solid ${TOKENS.line}`, borderRadius: TOKENS.radius, padding: '14px 18px' }}>
          <SectionLabel color={TOKENS.teal}>Why this matters — your note</SectionLabel>
          <div style={{ fontSize: 14, lineHeight: 1.55, fontStyle: 'italic', marginTop: 6 }}>"{goal.enthusiasm}"</div>
        </div>

        {!calm && (
          <ZenLine app={app} chips={[
            { chip: 'Why this matters', label: goal.name, context: `Goal: ${goal.name}. Outcome: ${goal.overarching}. The user's own note: "${goal.enthusiasm}". Current phase: ${ph?.name} — ${ph?.description}. Projects: ${goalProjects.map(p => p.name).join(', ') || 'none yet'}.`, seed: `Remind me why "${goal.name}" matters to me, in my own words.` },
            { chip: 'Suggest a next step', label: goal.name, context: `Goal: ${goal.name}. Outcome: ${goal.overarching}. Current phase: ${ph?.name}. Projects: ${goalProjects.map(p => `${p.name} (${Math.round(p.progress * 100)}%)`).join(', ') || 'none yet'}.`, seed: `What's one small, concrete next step I could take on "${goal.name}" this week?` },
          ]}>
            {isPrimary ? <>This is your <b>primary goal</b>. Want to reconnect with why it matters, or find the next small step?</> : <>A goal you're nurturing. Reconnect with why it matters, or find the next small step.</>}
          </ZenLine>
        )}
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {goal.phases.map(p => {
            ...
            return (
              <button key={p.id} onClick={() => setPhase(p.id)} style={{ ...btnReset, padding: '6px 12px', borderRadius: 999, background: phase === p.id ? goal.color : TOKENS.surface, color: phase === p.id ? '#fff' : TOKENS.sub, border: `1px solid ${phase === p.id ? goal.color : TOKENS.line}`, fontSize: 12, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
        ...
        {!calm && (
          <div style={{ marginTop: 8, background: TOKENS.surface, border: `1px solid ${TOKENS.line}`, borderRadius: TOKENS.radius, padding: '16px 18px' }}>
            ...
            <div style={{ fontSize: 13.5, lineHeight: 1.6, marginTop: 10, color: TOKENS.inkSoft }}>
              {isPrimary
                ? "You've been steady on this one — nice to see. The next honest thing is making progress; the rest is polish."
                : "This one's been quiet for a while. Even one small move would bring it back to life."}
            </div>
          </div>
        )}
      </div>
