...
import { TaskRow, SectionLabel } from '../shared/primitives.jsx';
import { Plus, Close, Edit, Trash, MoreV, Grid, Tag, Home, Heart, Flame } from '../shared/icons.jsx';
import { Bag, Wallet, Leaf, Calendar } from '../shared/icons.jsx';
  ...
  return (
    <div style={{ position: 'relative', flexShrink: 0 }}>
      <button onClick={() => setOpen(o => !o)} aria-label="Category options"
        style={{ ...btnReset, width: 28, height: 28, borderRadius: 999, display: 'grid', placeItems: 'center', color: open ? TOKENS.ink : TOKENS.sub, background: open ? TOKENS.bg : 'transparent' }}>
        <MoreV size={15} />
      </button>
      {open && (
        <>
          <div onClick={() => setOpen(false)} style={{ position: 'fixed', inset: 0, zIndex: 40 }} />
          <div style={{ position: 'absolute', top: 'calc(100% + 4px)', right: 0, zIndex: 41, width: 168, background: TOKENS.surface, border: `1px solid ${TOKENS.line}`, borderRadius: TOKENS.radius, boxShadow: '0 18px 44px rgba(20,30,20,0.2)', overflow: 'hidden', fontFamily: TOKENS.fontSans }}>
            {!confirm ? (
              <>
...
function CategoryQuickAdd({ cat, app }) {
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} style={{ ...btnReset, width: '100%', display: 'flex', alignItems: 'center', gap: 11, padding: '10px 16px', borderTop: `1px solid ${TOKENS.lineSoft}`, color: TOKENS.sub, fontSize: 12.5, cursor: 'pointer' }}>
...
function CategorySection({ cat, app, onNav, onEdit, onDelete, mobile = false, defaultOpen = true }) {
...
function CategoryEditorModal({ open, mode, cat, app, onClose }) {
  ...
  return (
    <div onClick={onClose} style={{ position: 'absolute', inset: 0, background: 'rgba(30,40,30,0.32)', backdropFilter: 'blur(4px)', display: 'grid', placeItems: 'center', zIndex: 90 }}>
      <div onClick={e => e.stopPropagation()} style={{ width: 'min(440px, 92%)', background: TOKENS.surface, borderRadius: TOKENS.radiusLg, border: `1px solid ${TOKENS.line}`, boxShadow: '0 40px 90px rgba(20,30,20,0.22)', overflow: 'hidden', fontFamily: TOKENS.fontSans, color: TOKENS.ink }}>
              ...
              {CATEGORY_ICON_KEYS.map(k => {
                ...
                return (
                  <button key={k} onClick={() => setIcon(k)} aria-label={'Icon ' + k}
                    style={{ ...btnReset, width: 36, height: 36, borderRadius: TOKENS.radius, display: 'grid', placeItems: 'center', cursor: 'pointer', background: sel ? color : TOKENS.bg, color: sel ? '#fff' : TOKENS.sub, border: `1px solid ${sel ? color : TOKENS.line}` }}>
        ...
        <div style={{ padding: '14px 18px', borderTop: `1px solid ${TOKENS.line}`, display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          ...
          <button onClick={save} disabled={!canSave} style={{ ...btnReset, padding: '8px 18px', borderRadius: 999, background: canSave ? TOKENS.green : TOKENS.greenSoft, color: canSave ? '#fff' : TOKENS.sub, fontSize: 12.5, fontWeight: 500, cursor: canSave ? 'pointer' : 'default', opacity: canSave ? 1 : 0.7 }}>
...
export default function CategoriesScreen() {
  ...
  const twoCol = !mobile && !categoryId;
  ...
  return (
    <Screen app={app} active={categoryId ? 'categories:' + categoryId : 'categories'} onNav={onNav} frame={frame}
      ...
      <div style={{ padding: '22px 30px 40px', maxWidth: twoCol ? 'none' : 760, margin: twoCol ? 0 : undefined }}>
        ...
        <div style={{ display: twoCol ? 'grid' : 'flex', gridTemplateColumns: twoCol ? '1fr 1fr' : undefined, flexDirection: twoCol ? undefined : 'column', gap: 14, alignItems: twoCol ? 'start' : undefined }}>
          {focusOne.map(cat => (
          ...
          {!categoryId && (
            <button onClick={() => setEditor({ mode: 'new' })} style={{ ...btnReset, padding: '12px 16px', border: `1px dashed ${TOKENS.line}`, borderRadius: TOKENS.radius, color: TOKENS.sub, fontSize: 13, display: 'inline-flex', alignItems: 'center', gap: 8, alignSelf: 'flex-start', gridColumn: twoCol ? '1 / -1' : undefined, width: twoCol ? 'auto' : undefined }}>
