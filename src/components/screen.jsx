...
function MobileMoreSheet({ app, active, onNav, onClose }) {
...
export function Screen({ frame, app, active, onNav, onAsk, title, subtitle, crumbs, headerRight, children }) {
  ...
  const ask = () => { onAsk ? onAsk() : setZen(true); };
      ...
      <div ref={rootRef} className={app.dyslexiaFont ? 'dys-font' : undefined} style={{ width: 360, height: 760, background: TOKENS.bg, color: TOKENS.ink,
        ...
        <header style={{ padding: '40px 18px 10px', borderBottom: `1px solid ${TOKENS.line}`, background: TOKENS.surface }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <NotificationsBell app={app} align="right" narrow/>
        ...
        <AskZenOverlay open={zen} onClose={()=>setZen(false)} app={app}/>
  ...
  return (
    <div ref={rootRef} className={app.dyslexiaFont ? 'dys-font' : undefined} style={{ width: '100%', height: '100%', background: TOKENS.bg, color: TOKENS.ink,
      ...
      <main data-tour="main" style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        <TopBar crumbs={crumbs} title={title} subtitle={subtitle} right={headerRight} app={app} onAsk={ask}/>
